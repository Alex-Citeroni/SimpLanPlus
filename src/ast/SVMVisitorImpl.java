...
import parser.SVM.SVMBaseVisitor;
...
// Questa classe permette di modificare la visita standard del bytecode generato da SimpLanPlus dei file .asm.
public class SVMVisitorImpl extends SVMBaseVisitor<Void> {
    ...
    public Void visitAssembly(SVMParser.AssemblyContext ctx) {
        visitChildren(ctx);
        labelRef.keySet().forEach(refAdd -> code[refAdd] = labelAdd.get(labelRef.get(refAdd)));
        return null;
    }
